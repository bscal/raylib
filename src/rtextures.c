...
Image Raylib_LoadImage(const char *fileName)
{
    Image image = { 0 };
...
Image LoadImageAnim(const char *fileName, int *frames)
{
    ...
    {
        image = Raylib_LoadImage(fileName);
        frameCount = 1;
    }
...
Image ImageFromImage(Image image, Raylib_Rectangle rec)
{
    Image result = { 0 };
...
void ImageCrop(Image *image, Raylib_Rectangle crop)
{
...
Image ImageTextEx(Font font, const char *text, float fontSize, float spacing, Color tint)
{
    ...
    for (int i = 0; i < size;)
    {
        ...
        else
        {
            ...
            {
                Raylib_Rectangle rec = { (float)(textOffsetX + font.glyphs[index].offsetX), (float)(textOffsetY + font.glyphs[index].offsetY), (float)font.recs[index].width, (float)font.recs[index].height };
                ImageDraw(&imText, font.glyphs[index].image, (Raylib_Rectangle){ 0, 0, (float)font.glyphs[index].image.width, (float)font.glyphs[index].image.height }, rec, tint);
            }
...
void ImageResizeCanvas(Image *image, int newWidth, int newHeight, int offsetX, int offsetY, Color fill)
{
    ...
    {
        Raylib_Rectangle srcRec = { 0, 0, (float)image->width, (float)image->height };
...
void ImageAlphaCrop(Image *image, float threshold)
{
    ...
    Raylib_Rectangle crop = GetImageAlphaBorder(*image, threshold);
...
// NOTE: Threshold is defined as a percentage: 0.0f -> 1.0f
Raylib_Rectangle GetImageAlphaBorder(Image image, float threshold)
{
    Raylib_Rectangle crop = { 0 };
    ...
    if (pixels != NULL)
    {
        ...
        {
            crop = (Raylib_Rectangle){ (float)xMin, (float)yMin, (float)((xMax + 1) - xMin), (float)((yMax + 1) - yMin) };
        }
...
{
    ImageDrawRectangleRec(dst, (Raylib_Rectangle){ (float)posX, (float)posY, (float)width, (float)height }, color);
}
...
// Draw rectangle within an image
void ImageDrawRectangleRec(Image *dst, Raylib_Rectangle rec, Color color)
{
    // Security check to avoid program crash
    if ((dst->data == NULL) || (dst->width == 0) || (dst->height == 0)) return;
...
void ImageDrawRectangleLines(Image *dst, Raylib_Rectangle rec, int thick, Color color)
{
...
void ImageDraw(Image *dst, Image src, Raylib_Rectangle srcRec, Raylib_Rectangle dstRec, Color tint)
{
    // Security check to avoid program crash
    ...
    else
    {
        ...
        if (((int)srcRec.width != (int)dstRec.width) || ((int)srcRec.height != (int)dstRec.height))
        {
            ...
            srcRec = (Raylib_Rectangle){ 0, 0, (float)srcMod.width, (float)srcMod.height };
        ...
        if ((dst->mipmaps > 1) && (src.mipmaps > 1))
        {
            ...
            Raylib_Rectangle mipmapSrcRec = srcRec;
            mipmapSrcRec.width /= 2;
            mipmapSrcRec.height /= 2;
            mipmapSrcRec.x /= 2;
            mipmapSrcRec.y /= 2;

            Raylib_Rectangle mipmapDstRec = dstRec;
...
void ImageDrawTextEx(Image *dst, Font font, const char *text, Vector2 position, float fontSize, float spacing, Color tint)
{
    ...
    Raylib_Rectangle srcRec = { 0.0f, 0.0f, (float)imText.width, (float)imText.height };
    Raylib_Rectangle dstRec = { position.x, position.y, (float)imText.width, (float)imText.height };
...
Texture2D LoadTexture(const char *fileName)
{
    ...
    Image image = Raylib_LoadImage(fileName);
...
TextureCubemap LoadTextureCubemap(Image image, int layout)
{
    ...
    if (layout != CUBEMAP_LAYOUT_AUTO_DETECT)
    {
        ...
        Raylib_Rectangle faceRecs[6] = { 0 };      // Face source rectangles

        for (int i = 0; i < 6; i++) faceRecs[i] = (Raylib_Rectangle){ 0, 0, (float)size, (float)size };
        ...
        else
        {
            ...
            for (int i = 0; i < 6; i++) ImageDraw(&faces, mipmapped, faceRecs[i], (Raylib_Rectangle){ 0, (float)size*i, (float)size, (float)size }, WHITE);
...
// NOTE: pixels data must match texture.format
void UpdateTextureRec(Texture2D texture, Raylib_Rectangle rec, const void *pixels)
{
...
void DrawTextureEx(Texture2D texture, Vector2 position, float rotation, float scale, Color tint)
{
    ...
    Raylib_Rectangle dest = { position.x, position.y, (float)texture.width*scale, (float)texture.height*scale };
...
void DrawTextureRec(Texture2D texture, Raylib_Rectangle source, Vector2 position, Color tint)
{
    Raylib_Rectangle dest = { position.x, position.y, fabsf(source.width), fabsf(source.height) };
    Vector2 origin = { 0.0f, 0.0f };
...
void DrawTexturePro(Texture2D texture, Raylib_Rectangle source, Raylib_Rectangle dest, Vector2 origin, float rotation, Color tint)
{
...
void DrawTextureNPatch(Texture2D texture, NPatchInfo nPatchInfo, Raylib_Rectangle dest, Vector2 origin, float rotation, Color tint)
{
    if (texture.id > 0)
    {
        float width = (float)texture.width;
