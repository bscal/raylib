...
*           If no default font loaded, Raylib_DrawTextEx() and MeasureTextEx() are required.
*
...
// NOTE: Default font is loaded on InitWindow() and disposed on Raylib_CloseWindow() [module: core]
...
extern void LoadFontDefault(void)
{
    ...
    // NOTE: This memory must be freed at end! --> Done by Raylib_CloseWindow()
    defaultFont.glyphs = (GlyphInfo *)RL_CALLOC(defaultFont.glyphCount, sizeof(GlyphInfo));
    defaultFont.recs = (Raylib_Rectangle *)RL_CALLOC(defaultFont.glyphCount, sizeof(Raylib_Rectangle));
...
Font LoadFontFromImage(Image image, Color key, int firstChar)
{
    ...
    Raylib_Rectangle tempCharRecs[MAX_GLYPHS_FROM_IMAGE] = { 0 };
    ...
    font.recs = (Raylib_Rectangle *)RL_MALLOC(font.glyphCount*sizeof(Raylib_Rectangle));
...
#if defined(SUPPORT_FILEFORMAT_TTF) || defined(SUPPORT_FILEFORMAT_BDF)
Image GenImageFontAtlas(const GlyphInfo *glyphs, Raylib_Rectangle **glyphRecs, int glyphCount, int fontSize, int padding, int packMethod)
{
    Image atlas = { 0 };
    ...
    *glyphRecs = NULL;
    ...
    Raylib_Rectangle *recs = (Raylib_Rectangle *)RL_MALLOC(glyphCount*sizeof(Raylib_Rectangle));
...
#define SUPPORT_COMPRESSED_FONT_ATLAS
    ...
    byteCount += sprintf(txtData + byteCount, "static Raylib_Rectangle fontRecs_%s[%i] = {\n", fileNamePascal, font.glyphCount);
    for (int i = 0; i < font.glyphCount; i++)
    {
...
//#define SUPPORT_FONT_DATA_COPY
    ...
    byteCount += sprintf(txtData + byteCount, "    font.recs = (Raylib_Rectangle *)malloc(font.glyphCount*sizeof(Raylib_Rectangle));\n");
    byteCount += sprintf(txtData + byteCount, "    memcpy(font.recs, fontRecs_%s, font.glyphCount*sizeof(Raylib_Rectangle));\n\n", fileNamePascal);
...
void DrawFPS(int posX, int posY)
{
    ...
    Raylib_DrawText(TextFormat("%2i FPS", fps), posX, posY, 20, color);
}

// Draw text (using default font)
// NOTE: fontSize work like in any drawing program but if fontSize is lower than font-base-size, then font-base-size is used
// NOTE: chars spacing is proportional to fontSize
void Raylib_DrawText(const char *text, int posX, int posY, int fontSize, Color color)
{
    ...
    if (GetFontDefault().texture.id != 0)
    {
        ...
        Raylib_DrawTextEx(GetFontDefault(), text, position, (float)fontSize, (float)spacing, color);
    }
}

// Draw text using Font
// NOTE: chars spacing is NOT proportional to fontSize
void Raylib_DrawTextEx(Font font, const char *text, Vector2 position, float fontSize, float spacing, Color tint)
{
...
void DrawTextPro(Font font, const char *text, Vector2 position, Vector2 origin, float rotation, float fontSize, float spacing, Color tint)
{
    rlPushMatrix();
        ...
        Raylib_DrawTextEx(font, text, (Vector2){ 0.0f, 0.0f }, fontSize, spacing, tint);
...
void DrawTextCodepoint(Font font, int codepoint, Vector2 position, float fontSize, Color tint)
{
    ...
    Raylib_Rectangle dstRec = { position.x + font.glyphs[index].offsetX*scaleFactor - (float)font.glyphPadding*scaleFactor,
    ...
    Raylib_Rectangle srcRec = { font.recs[index].x - (float)font.glyphPadding, font.recs[index].y - (float)font.glyphPadding,
...
// NOTE: If codepoint is not found in the font it fallbacks to '?'
Raylib_Rectangle GetGlyphAtlasRec(Font font, int codepoint)
{
    Raylib_Rectangle rec = { 0 };
...
static Font LoadBMFont(const char *fileName)
{
    ...
    for (int i = 0; i < pageCount; i++)
    {
        imFonts[i] = Raylib_LoadImage(TextFormat("%s/%s", GetDirectoryPath(fileName), imFileName[i]));
    ...
    if (pageCount > 1)
    {
        ...
        {
            Raylib_Rectangle srcRec = { 0.0f, 0.0f, (float)imWidth, (float)imHeight };
            Raylib_Rectangle destRec = { 0.0f, (float)imHeight*(float)i, (float)imWidth, (float)imHeight };
    ...
    font.recs = (Raylib_Rectangle *)RL_MALLOC(glyphCount*sizeof(Raylib_Rectangle));
    ...
    for (int i = 0; i < glyphCount; i++)
    {
        ...
        {
            // Get character rectangle in the font atlas texture
            font.recs[i] = (Raylib_Rectangle){ (float)charX, (float)charY + (float)imHeight*pageID, (float)charWidth, (float)charHeight };
